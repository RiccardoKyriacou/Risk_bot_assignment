#Code to analyse risk from given list of lists
...
def get_risk(lst):
    ...
    for i in lst:
        cancer_count += i
    risk = cancer_count / len(lst) * 100 #Calcualte the risk 
    srisk = ""
    if risk <= 25: #Risk needs to be in string formatt so define cutoffs and return a string
        srisk += "LOW"
    elif 25 < risk < 75:
        srisk += "MEDIUM"
    elif risk >= 75:
...
def main():
    risk_lst = [] #List of the risks for each row of the table
    cancer_lst = get_cancer_lst_1(cancer_data_list) 
    for lst in cancer_lst: 
        risk = get_risk(lst)    
        risk_lst.append(risk)
    print(f"Vegetraian: Yes, Years Smoking: 0-4, Age: <=40 then risk is {risk_lst[0]}") #Index list to get correct risk 
